...
However, I reccomend piCore OS for its supreme stability and provide all necessary piCore 12.x ARMv7L dependencies as stripped bespoke piCore extensions with only the files required by this application, including a complete list of dependencies for /mnt/mmcblk0p2/onboot.lst here: https://github.com/Xenodius/ASI_AmpyDisplay_piCore12-dependencies. Any questions about piCore can be answered by searching the forum (http://forum.tinycorelinux.net/index.php/). If you're interested in using piCore and using identical hardware, but the setup is too complex for you, start with this fully configured image; https://drive.google.com/file/d/1ZPHdDI0X1PqNe5SemjnQvdbJ-f5fof-r/view?usp=sharing. Just flash to sd, edit your vehicle parameters in the `/home/tc/.X.d/ampy` startup script, connect to your home wifi then run `filetool.sh -b` to save WPA keys locally for easy future updates via SSH, expand the filesystem as described in the official TinyCore readme, and you're set. Git package can be loaded on-demand to update the application via:
```tce-load -i git
cd /mnt/mmcblk0p2/ASI_AmpyDisplay
sudo git pull
sudo git checkout master # Or dev if adventureous
```
